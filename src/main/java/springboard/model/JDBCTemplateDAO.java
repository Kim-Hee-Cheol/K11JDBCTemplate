...
- int queryForInt(String sql) 혹은 queryForInt(String, Object[] args)
	: 쿼리문의 실행결과가 숫자를 반환하는 select계열의
	쿼리문에 사용한다.
- Object queryForObject(String sql, RowMapper rowMapeer)
	혹은 Object queryForObject(String sql, Object[] args, RowMapper rowMapper)
	: 하나의 레코드를 반환하는 select계열의 쿼리문 실행 시 사용된다.
...
public class JDBCTemplateDAO {
	...
	public JDBCTemplateDAO() {
		/*
			컨트롤레에서 @Autowired를 통해  자동주입 받았던 빈을
	...
	public ArrayList<SpringBbsDTO> list(Map<String, Object> map){
		...
		String sql = ""
			...
			sql +=" ORDER BY bgroup DESC, bstep ASC"; //답변글 적용 시 
	...
	public int password(String idx, String pass) {
		...
		에외처리를 하고있다. queryForObject()는 반환되는 행이
		1개일 때만 정상 동작한다.
		 */
		try {
			SpringBbsDTO dto = template.queryForObject(sql,
					new BeanPropertyRowMapper<SpringBbsDTO>(
						SpringBbsDTO.class));
			/*
			idx와 pass에 해당하는 게시물이 정상적으로 가져와졌을 때는
			해당 idx값을 반환값으로 사용한다. 
			 */
			retNum = dto.getIdx();
		}
