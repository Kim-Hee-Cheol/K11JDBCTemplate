...
public class BbsController {
	...
	/*
	스프링 어플리캐이션이 구동될 때 미리 생성된 JdbcTemplate타입의
	빈을 자동으로 주입받게된다.
	 */
	@Autowired
	public void setTemplate(JdbcTemplate template) {
		this.template = template;
		System.out.println("@Autowirded->JDBCTemplate 연결성공");
	...
	public String list(Model model, HttpServletRequest req) {
		...
		이른 커맨득개체로 전달하기 위해 model에 저장 후 매개변수로
		전달한다.
		 */
		model.addAttribute("req",req);
		
		/*
		컨트롤러는 사용자의 요청을 분석한 후 해당 요청에 맞는 서비스 객체만
		호출하고 실제 DAO의 호출이나 비지니스로직은 아래 Command객체가
		처리하게된다.
		 */
